...
export const CreateTaskModal: React.FC<{ isOpen: boolean; onClose: () => void; columnId: string; boardId: string }> = ({ isOpen, onClose, columnId }) => {
  const { register, handleSubmit } = useForm<{ title: string; description?: string }>();

  const onSubmit = async (data: { title: string; description?: string }) => {
    try {
      await taskService.createTask(columnId, { title: data.title, description: data.description });
      onClose();
    } catch (e) {
