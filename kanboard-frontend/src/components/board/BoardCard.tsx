...
import { EyeIcon, TrashIcon, CalendarDaysIcon, RectangleStackIcon } from "@heroicons/react/24/outline";
      ...
      <div className="relative z-10">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-500">
              <div className="flex items-center space-x-1">
                ...
                <span>{new Date(board.createdAt).toLocaleDateString("tr-TR")}</span>
              </div>
              <div className="flex items-center space-x-1">
        ...
        <div className="mb-4">
          <div className="flex justify-between text-xs text-gray-500 mb-1">
            <span>İlerleme</span>
            <span>{taskCount > 0 ? Math.round((taskCount / 10) * 100) : 0}%</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div 
              className="bg-gradient-to-r from-blue-500 to-indigo-600 h-2 rounded-full transition-all duration-500"
              style={{ width: `${taskCount > 0 ? Math.min((taskCount / 10) * 100, 100) : 0}%` }}
            ></div>
          </div>
        </div>
